...
export function FinancePaymentsPage() {
  ...
  return <FinancePage eyebrow="Cash control" title="Supplier payments" description="Prepare payment vouchers, allocate approved invoice balances, and enforce maker-checker posting." actions={can.prepareFinance(role) ? <Button onClick={() => setCreating(true)}><Plus className="h-4 w-4" />New payment</Button> : undefined}>
    ...
    <PaymentModal open={creating} invoiceId={initialInvoiceId || undefined} onClose={() => setCreating(false)} /><PaymentDetail payment={selected} onClose={() => setSelected(null)} /><AllocationModal payment={allocating} onClose={() => setAllocating(null)} /><ReasonModal state={reason} pending={command.isPending && command.variables?.id === reason?.payment.id} onClose={() => setReason(null)} onConfirm={(text) => reason && command.mutate({ id: reason.payment.id, action: reason.action, body: reason.action === 'reverse' ? { reason: text, idempotency_key: idempotencyKey('payment-reverse') } : { reason: text } })} />
  </FinancePage>;
}

function PaymentDetail({ payment, onClose }: { payment: Payment | null; onClose: () => void }) {
  return <FormModal open={!!payment} title={`Payment voucher / ${payment?.number || ''}`} onClose={onClose}>
    {payment ? <div className="grid gap-4 text-sm">
      <div className="grid gap-3 rounded-lg border border-border bg-background p-3 sm:grid-cols-2"><p><span className="block text-xs text-muted">Supplier</span><strong>{payment.supplier_name || 'Supplier advance'}</strong></p><p><span className="block text-xs text-muted">Status</span><Status value={payment.status} /></p><p><span className="block text-xs text-muted">Payment account</span><strong>{payment.source_account_name}</strong></p><p><span className="block text-xs text-muted">Payment date</span><strong>{formatDate(payment.payment_date)}</strong></p><p><span className="block text-xs text-muted">Amount</span><strong>{formatMoney(payment.amount, payment.currency_code)}</strong></p><p><span className="block text-xs text-muted">Unallocated</span><strong>{formatMoney(payment.unallocated_amount, payment.currency_code)}</strong></p></div>
      <section><h3 className="font-bold">Invoice allocations</h3><div className="mt-2 grid gap-2">{payment.allocations.length ? payment.allocations.map((allocation) => <div key={allocation.id} className="flex flex-wrap items-center justify-between gap-2 border border-border p-3"><span><strong>{allocation.invoice_number}</strong><small className="block text-muted">{allocation.status.replace(/_/g, ' ')}</small></span><strong>{formatMoney(allocation.amount, payment.currency_code)}</strong></div>) : <p className="text-muted">No invoice has been allocated to this voucher.</p>}</div></section>
      {payment.notes ? <section><h3 className="font-bold">Notes</h3><p className="mt-1 text-muted">{payment.notes}</p></section> : null}
    </div> : null}
  </FormModal>;
}

function PaymentAttentionPanel({ submitted, submittedCount, approved, approvedCount, onShow }: { submitted: Payment[]; submittedCount: number; approved: Payment[]; approvedCount: number; onShow: (status: string) => void }) {
  return <section className="overflow-hidden border border-primary/25 bg-white shadow-panel">
    <div className="flex flex-wrap items-start justify-between gap-2 border-b border-primary/15 bg-primary/5 px-3 py-2.5 sm:px-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-primary">Action required</p><h3 className="mt-0.5 font-black">Your payment decisions</h3><p className="mt-0.5 text-xs text-muted">Approve prepared vouchers first; then post the approved payments.</p></div><strong className="rounded-full bg-primary px-2.5 py-1 text-xs text-white">{submittedCount + approvedCount} open</strong></div>
    <div className="grid divide-y divide-border lg:grid-cols-2 lg:divide-x lg:divide-y-0">
      <AttentionLane title="Awaiting your approval" count={submittedCount} empty="No submitted payment vouchers need approval." onShow={() => onShow('SUBMITTED')}>{submitted.map((payment) => <div key={payment.id} className="border-t border-border py-2 first:border-t-0"><strong className="text-sm">{payment.number}</strong><p className="text-xs text-muted">{payment.supplier_name || 'Supplier advance'} · {formatMoney(payment.amount, payment.currency_code)}</p></div>)}</AttentionLane>
      <AttentionLane title="Approved - ready to post" count={approvedCount} empty="No approved payment vouchers are waiting to be posted." onShow={() => onShow('APPROVED')}>{approved.map((payment) => <div key={payment.id} className="border-t border-border py-2 first:border-t-0"><strong className="text-sm">{payment.number}</strong><p className="text-xs text-muted">{payment.supplier_name || 'Supplier advance'} · {formatMoney(payment.amount, payment.currency_code)}</p></div>)}</AttentionLane>
    </div>
  </section>;
}
...
function PaymentModal({ open, invoiceId, onClose }: { open: boolean; invoiceId?: number; onClose: () => void }) {
  ...
  const accounts = useQuery({ queryKey: ['finance', 'cash-accounts', 'payments'], queryFn: () => financeApi.cashAccounts({ page_size: 100, is_active: true }) });
  const systemCashAccounts = useQuery({ queryKey: qk.financeAccounts({ page_size: 100, system_key: 'CASH' }), queryFn: () => financeApi.accounts({ page_size: 100, system_key: 'CASH', is_active: true }) });
  ...
  const paymentAccounts = useMemo(() => {
    ...
    (systemCashAccounts.data?.results || []).forEach((account) => options.set(account.id, { value: account.id, label: `${account.code} / ${account.name}` }));
    (accounts.data?.results || []).forEach((account) => options.set(account.account, { value: account.account, label: `${account.code} / ${account.name} · ${account.currency_code}` }));
    return [...options.values()];
  }, [accounts.data?.results, systemCashAccounts.data?.results]);
