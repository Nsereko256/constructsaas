...
function InvoiceActions({ invoice, role, pendingAction, view, run, requestApprovalOverride, reason, deleteDraft }: { invoice: SupplierInvoice; role: ReturnType<typeof useAuth>['role']; pendingAction: string | null; view: () => void; run: (action: string, body?: unknown) => void; requestApprovalOverride: () => void; reason: (action: NonNullable<ReasonAction>['action']) => void; deleteDraft: () => void }) {
  ...
  const primaryAction = can.prepareFinance(role) && ['POSTED', 'PARTIALLY_PAID'].includes(invoice.status) && Number(invoice.balance) > 0
        ...
        ? <Button size="sm" loading={pendingAction === 'run-match'} loadingLabel="Matching" onClick={() => run('run-match', { idempotency_key: idempotencyKey('match') })} disabled={pending}><Scale className="h-3.5 w-3.5" />{invoice.status === 'MATCH_EXCEPTION' ? 'Run match again' : 'Run match'}</Button>
          : can.manageFinance(role) && ['MATCHED','VERIFIED'].includes(invoice.status)
            ...
            : can.manageFinance(role) && invoice.status === 'APPROVED'
              ? <Button size="sm" loading={pendingAction === 'post'} loadingLabel="Posting" disabled={pending} onClick={() => run('post', { idempotency_key: idempotencyKey('invoice-post') })}><Upload className="h-3.5 w-3.5" />Post</Button>
              : null;
  const hasSecondary = ((can.prepareFinance(role) || role === 'admin') && invoice.status === 'DRAFT')
    || (role === 'finance_manager' && invoice.status === 'MATCH_EXCEPTION')
    || (can.manageFinance(role) && ['SUBMITTED','MATCH_EXCEPTION','MATCHED','VERIFIED'].includes(invoice.status));
...
function InvoiceModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <FormModal open={open} title="Capture supplier invoice" onClose={onClose}><form className="grid gap-3" onSubmit={(event: FormEvent) => { event.preventDefault(); mutation.mutate(); }}>
    ...
    <div className="grid gap-3 md:grid-cols-3"><Field label="Supplier invoice number" required><input className={inputClass} value={form.invoice_number} onChange={(event) => setForm((current) => ({ ...current, invoice_number: event.target.value }))} /></Field><Field label="Invoice date" required><input className={inputClass} type="date" value={form.invoice_date} onChange={(event) => setForm((current) => ({ ...current, invoice_date: event.target.value }))} /></Field><Field label="Due date"><input className={inputClass} type="date" min={form.invoice_date} value={form.due_date} onChange={(event) => setForm((current) => ({ ...current, due_date: event.target.value }))} /></Field><Field label="Currency" required><select className={inputClass} value={form.currency} onChange={(event) => setForm((current) => ({ ...current, currency: event.target.value }))}>{currencies.data?.results.map((currency) => <option key={currency.id} value={currency.code}>{currency.code} / {currency.name}</option>)}</select></Field><Field label="Exchange rate" required><input className={inputClass} type="number" min="0.000001" step="0.000001" value={form.exchange_rate} onChange={(event) => setForm((current) => ({ ...current, exchange_rate: event.target.value }))} /></Field></div>
