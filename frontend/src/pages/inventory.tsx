...
import { AlertTriangle, ArrowDownToLine, ArrowUpFromLine, Boxes, Coins, Eye, MoreHorizontal, PackageCheck, PackageOpen, Pencil, Plus, ReceiptText, Route, Trash2, TrendingUp } from 'lucide-react';
...
export function InventoryPage() {
  ...
  const appliedFilterCount = [list.search, list.filters.category, list.filters.low_stock, list.filters.is_active !== 'true'].filter(Boolean).length;
  ...
  return (
    <div className="operations-reference inventory-reference grid gap-3">
      ...
      {lowStock.length ? <div className="ops-alert"><span className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-warning" /><strong>{lowStock.length} stock action{lowStock.length === 1 ? ' requires' : 's require'} attention.</strong></span><button className="ops-link" onClick={() => list.setFilter('low_stock', 'true')}>View stock queue →</button></div> : null}
