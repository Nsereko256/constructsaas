...
import { AlertTriangle, Boxes, CheckCircle2, Coins, Download, Eye, PackageCheck, PackageOpen, Pencil, Plus, ReceiptText, Route, Trash2, TrendingUp, Upload } from 'lucide-react';
...
export function InventoryPage() {
  ...
  return (
    <div className="operations-reference inventory-reference grid gap-3">
      <header className="ops-page-head"><div className="ops-page-titlebar"><div><h1>Inventory</h1><p>Control material balances, locations, stock movements and site custody.</p></div><div className="ops-actions"><Button variant="secondary" onClick={() => void download('xlsx')}><Download className="h-4 w-4" />Export</Button>{can.createMovement(role) ? <Button variant="secondary" onClick={() => setImportOpen(true)}><Upload className="h-4 w-4" />Import opening stock</Button> : null}{['storekeeper', 'admin'].includes(role || '') ? <Button asChild><Link to="/procurement/requests?action_queue=my_requests"><PackageOpen className="h-4 w-4" />Issue stock</Link></Button> : null}{allowed ? <><Button variant="secondary" onClick={() => setCategoryOpen(true)}><Plus className="h-4 w-4" />Category</Button><Button onClick={() => setMaterialOpen(true)}><Plus className="h-4 w-4" />Material</Button></> : null}</div></div><div className="ops-tabs"><WorkspaceTabs links={[{ href: '/inventory', label: 'Stock', icon: Boxes }, { href: '/inventory/bin-locations', label: 'Bin locations', icon: ReceiptText }, { href: '/inventory/movements', label: 'Movements', icon: Route }, { href: '/inventory/site-custody', label: 'Site custody', icon: ReceiptText }]} /></div></header>
