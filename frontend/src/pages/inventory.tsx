...
import { AlertTriangle, ArrowDownToLine, ArrowUpFromLine, Boxes, CheckCircle2, Coins, Eye, PackageCheck, PackageOpen, Pencil, Plus, ReceiptText, Route, Trash2, TrendingUp } from 'lucide-react';
...
export function InventoryPage() {
  ...
  const columns: ColumnDef<Material>[] = [
    ...
    { header: 'Status', cell: ({ row }) => <Badge tone={row.original.is_low_stock ? 'warning' : 'success'}>{row.original.is_low_stock ? 'Low stock' : 'Healthy'}</Badge> },
    {
      ...
      header: '',
      cell: ({ row }) =>
        <div className="ops-row-actions"><Button variant="ghost" size="sm" onClick={() => setSelected(row.original)}><Eye className="h-4 w-4" />View</Button>{allowed ? <Button variant="ghost" size="sm" onClick={() => setMaterialOpen(row.original)}><Pencil className="h-4 w-4" />Edit</Button> : null}{canDeactivate ? <Button variant="ghost" size="sm" onClick={() => { if (window.confirm(`Deactivate ${row.original.name}? Existing stock history will be preserved.`)) deactivate.mutate(row.original.id); }}><Trash2 className="h-4 w-4" />Deactivate</Button> : null}</div>,
    },
  ];

  return (
    <div className="operations-reference inventory-reference grid gap-3">
      <header className="ops-page-head"><div className="ops-page-titlebar"><div><h1>Inventory</h1><p>Control material balances, locations, stock movements and site custody.</p></div><div className="ops-actions"><Button variant="secondary" onClick={() => void download('xlsx')}><ArrowUpFromLine className="h-4 w-4" />Export</Button>{can.createMovement(role) ? <Button variant="secondary" onClick={() => setImportOpen(true)}><ArrowDownToLine className="h-4 w-4" />Import opening stock</Button> : null}{['storekeeper', 'admin'].includes(role || '') ? <Button asChild><Link to="/procurement/requests?action_queue=my_requests"><PackageOpen className="h-4 w-4" />Issue stock</Link></Button> : null}{allowed ? <><Button variant="secondary" onClick={() => setCategoryOpen(true)}><Plus className="h-4 w-4" />Category</Button><Button onClick={() => setMaterialOpen(true)}><Plus className="h-4 w-4" />Material</Button></> : null}</div></div><div className="ops-tabs"><WorkspaceTabs links={[{ href: '/inventory', label: 'Stock', icon: Boxes }, { href: '/inventory/bin-locations', label: 'Bin locations', icon: ReceiptText }, { href: '/inventory/movements', label: 'Movements', icon: Route }, { href: '/inventory/site-custody', label: 'Site custody', icon: ReceiptText }]} /></div></header>
