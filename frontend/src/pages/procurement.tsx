...
export function ProcurementPage() {
  ...
  const coreQueries = [requests, orders, receipts, workflow];
  if (coreQueries.some((query) => query.isLoading)) return <div className="procurement-reference"><h1>Procurement</h1><ProcurementTabs /><p role="status" className="text-sm text-muted">Loading your procurement queues…</p><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[0, 1, 2, 3].map((key) => <Skeleton key={key} className="h-24" />)}</div><Skeleton className="h-64" /></div>;
  if (coreQueries.some((query) => query.isError)) return <div className="procurement-reference"><h1>Procurement</h1><ProcurementTabs /><div role="alert" className="card-surface p-5"><h2 className="font-semibold">Could not load the procurement overview</h2><p className="mt-1 text-sm text-muted">Retry to see current counts, or use the workspace navigation to open a register.</p><Button className="mt-3" variant="secondary" onClick={() => { coreQueries.filter((query) => query.isError).forEach((query) => void query.refetch()); }} loading={coreQueries.some((query) => query.isFetching)}>Try again</Button></div></div>;
  ...
  return (
    <div className="procurement-reference">
      ...
      {actionTotal > 0 ? <section className="procurement-alert"><AlertTriangle size={17} /><strong>{actionTotal} actions require your attention</strong><Link to="/procurement/requests?action_queue=my_requests">View priority queue <ChevronRight size={14} /></Link></section> : null}
      ...
      <section className="procurement-content-grid"><div data-pagination-region className="procurement-orders procurement-panel"><div className="procurement-panel-heading"><h2>Active purchase orders</h2></div><div className="procurement-order-tabs">{[['all', 'All', openOrders.length], ['approval', 'Awaiting approval', pendingOrders.length], ['issued', 'Issued', issuedOrders.length], ['partial', 'Part delivered', orderRows.filter((order) => order.status === 'PARTIAL').length]].map(([value, label, count]) => <button type="button" className={activeTab === value ? 'active' : ''} key={String(value)} onClick={() => { setActiveTab(value as typeof activeTab); setPage(1); }}>{label} <b>{count}</b></button>)}</div><RegisterFilters className="procurement-filters" activeCount={[search, project, supplier, status].filter(Boolean).length} onClear={() => { setSearch(''); setProject(''); setSupplier(''); setStatus(''); setPage(1); }}><label><Search size={14} /><input aria-label="Search purchase orders" placeholder="Search purchase orders" value={search} onChange={(event) => setFilter(setSearch, event.target.value)} /></label><select aria-label="Filter purchase orders by project" className={inputClass} value={project} onChange={(event) => setFilter(setProject, event.target.value)}><option value="">Project</option>{projectOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><select aria-label="Filter purchase orders by supplier" className={inputClass} value={supplier} onChange={(event) => setFilter(setSupplier, event.target.value)}><option value="">Supplier</option>{supplierOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><select aria-label="Filter purchase orders by status" className={inputClass} value={status} onChange={(event) => setFilter(setStatus, event.target.value)}><option value="">Status</option><option value="DRAFT">Draft</option><option value="PENDING">Awaiting approval</option><option value="ORDERED">Ordered</option><option value="DISPATCH_CONFIRMED">Dispatched</option><option value="PARTIAL">Part delivered</option></select></RegisterFilters><TableScroll label="Active purchase orders" className="procurement-table-wrap"><table className="procurement-table"><thead><tr><th>PO number</th><th>Project</th><th>Supplier</th><th>Value</th><th>Delivery</th><th>Status</th><th>Approval</th><th aria-label="Actions" /></tr></thead><tbody>{visibleOrders.map((order) => { const delivery = order.revised_delivery_date || order.supplier_confirmed_delivery_date || order.expected_delivery_date; return <tr key={order.id}><td><Link to={`/procurement/purchase-orders?search=${encodeURIComponent(order.number)}`}>{order.number}</Link></td><td>{order.project_name || 'Warehouse'}</td><td>{order.supplier_name || 'Not selected'}</td><td>{compactUGX(order.total_cost)}</td><td className={order.is_overdue ? 'overdue' : ''}>{order.is_overdue ? `Overdue · ${shortDate(delivery)}` : shortDate(delivery)}</td><td><Badge tone={poTone(order.status, order.is_overdue)}>{order.is_overdue ? 'Delayed' : order.status_display}</Badge></td><td><Badge tone={order.status === 'PENDING' || order.status === 'DRAFT' ? 'warning' : 'success'}>{order.status === 'PENDING' ? 'Awaiting approval' : order.status === 'DRAFT' ? 'Not submitted' : 'Approved'}</Badge></td><td><Link aria-label={`Open ${order.number}`} to={`/procurement/purchase-orders?search=${encodeURIComponent(order.number)}`}><MoreVertical size={16} /></Link></td></tr>; })}</tbody></table>{!visibleOrders.length ? <p className="procurement-empty">{orders.isLoading ? 'Loading purchase orders…' : 'No purchase orders match this view.'}</p> : null}</TableScroll><Pagination page={page} setPage={setPage} pageSize={4} data={{ count: filteredOrders.length, results: visibleOrders, previous: page > 1 ? 'previous' : null, next: page < pageCount ? 'next' : null }} itemLabel="purchase orders" /></div>
        <aside className="procurement-side-column"><section className="procurement-panel procurement-approvals"><div className="procurement-panel-heading"><h2>Approval queue</h2><Link to="/procurement/requests?action_queue=my_requests">View all</Link></div>{pendingRequests.slice(0, 3).map((request) => <ApprovalRow request={request} key={request.id} />)}{!pendingRequests.length ? <p className="procurement-empty">No material requests await approval.</p> : null}</section><section className="procurement-panel procurement-performance"><div className="procurement-panel-heading"><h2>Supplier performance</h2><Link to="/suppliers">View all</Link></div><PerformanceRow label="On-time delivery" value={onTimeRate} display={receivedWithDates.length ? `${onTimeRate}%` : 'No data'} /><PerformanceRow label="Quality acceptance" value={qualityRate} display={receiptQuantities.total ? `${qualityRate}%` : 'No data'} /><PerformanceRow label="Average lead time" value={Math.min(100, averageLeadTime / 30 * 100)} display={receivedWithLeadTime.length ? `${averageLeadTime} days` : 'No data'} /></section></aside></section>
    </div>
  );
}
