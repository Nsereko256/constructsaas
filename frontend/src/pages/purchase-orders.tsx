...
export function PurchaseOrdersPage() {
  ...
  const [financeHandoffOrder, setFinanceHandoffOrder] = useState<PurchaseOrder | null>(null);
  ...
  const openPurchaseOrderDetail = (event: MouseEvent<HTMLTableRowElement> | KeyboardEvent<HTMLTableRowElement>, orderId: number) => {
    const target = event.target as HTMLElement;
    if (target.closest('a, button, input, select, summary, textarea, label')) return;
...
function PurchaseOrderModal({ open, onClose, onCreated, initialPurchaseRequestId }: { open: boolean; onClose: () => void; onCreated: (order: PurchaseOrder) => void; initialPurchaseRequestId?: number }) {
  ...
  const mutation = useMutation<PurchaseOrder>({
    mutationFn: async () => {
      ...
      throw new Error('Select a manager-approved material request. Supplier pricing is entered here before Finance review.');
    },
    onSuccess: (order) => {
      toast.push({ title: 'Purchase order created', message: 'Send it to Finance now for budget review.', tone: 'success' });
      onClose();
      onCreated(order);
      void queryClient.invalidateQueries({ queryKey: ['purchase-orders'] });
      void queryClient.invalidateQueries({ queryKey: ['purchase-requests'] });
    ...
    <FormModal open={open} title="Create purchase order" onClose={onClose}>
      <form className="grid gap-3" onSubmit={(event: FormEvent) => { event.preventDefault(); mutation.mutate(); }}>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Field label="Manager-approved material request" required>
              <select className={inputClass} value={purchaseRequest} onChange={(event) => setPurchaseRequest(event.target.value)}>
                <option value="">Select manager-approved MR</option>
                {eligiblePrs.map((request: PurchaseRequest) => <option key={request.id} value={request.id}>{request.number} - {request.title}</option>)}
              </select>
              {!approvedPrs.isLoading && !eligiblePrs.length ? <p className="mt-1 text-xs text-warning">No manager-approved request is ready for a PO. Any available warehouse stock issue must be completed first.</p> : <p className="mt-1 text-xs text-muted">{eligiblePrs.length} manager-approved request{eligiblePrs.length === 1 ? '' : 's'} ready for purchase ordering. Finance reviews the final PO total before approval.</p>}
          </Field>
        ...
        {selectedPr ? (
          <Card>
            <CardHeader>
              ...
              <p className="text-xs text-muted">Only quantities still outstanding after stock issue are included. Fully issued lines are excluded automatically. Materials and quantities are locked to the manager-approved request balance; enter the agreed supplier prices for all remaining lines.</p>
            </CardHeader>
