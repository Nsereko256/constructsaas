...
export function PurchaseOrdersPage() {
  ...
  const rowAction = (order: PurchaseOrder) => {
    ...
    if (can.createPo(role) && ['DRAFT', 'PENDING'].includes(order.status)) return <div className="po-row-actions"><Button size="sm" className="po-next-action" loading={approve.isPending && approve.variables === order.id} loadingLabel="Approving" disabled={approve.isPending} onClick={() => approve.mutate(order.id)}>Approve PO</Button>{canEdit ? <Button size="sm" variant="secondary" className="po-next-action" onClick={() => setAmending(order)}><FilePenLine size={13} />{editLabel}</Button> : null}</div>;
