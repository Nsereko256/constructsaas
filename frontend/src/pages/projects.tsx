  ...
  return (
    <div className="projects-reference">
      ...
      <section className="projects-content-grid">
        <div data-pagination-region className="projects-portfolio projects-panel">
          <div className="projects-panel-heading"><h2>Project portfolio</h2></div>
          <div className="projects-portfolio-toolbar"><div className="projects-status-tabs">{[['', 'All', statusCounts.all], ['active', 'Active', statusCounts.active], ['planning', 'Planning', statusCounts.planning], ['completed', 'Completed', statusCounts.completed]].map(([value, label, count]) => <button type="button" key={String(value)} className={list.filters.status === value ? 'active' : ''} onClick={() => list.setFilter('status', String(value))}>{label} <b>{count}</b></button>)}</div></div><RegisterFilters className="projects-filters" activeCount={[list.search, list.filters.manager, list.filters.status].filter(Boolean).length} onClear={() => { list.setSearch(''); list.setFilter('manager', ''); list.setFilter('status', ''); }}><label className="projects-search"><Search size={14} /><input aria-label="Search projects" placeholder="Search projects" value={list.search} onChange={(event) => list.setSearch(event.target.value)} /></label><select aria-label="Filter projects by manager" className={inputClass} value={list.filters.manager} onChange={(event) => list.setFilter('manager', event.target.value)}><option value="">Manager</option>{managers.map((manager) => <option key={manager.id} value={manager.id || ''}>{manager.name}</option>)}</select><select aria-label="Filter projects by status" className={inputClass} value={list.filters.status} onChange={(event) => list.setFilter('status', event.target.value)}><option value="">Status</option><option value="planning">Planning</option><option value="active">Active</option><option value="on_hold">On hold</option><option value="completed">Completed</option></select><span className="projects-view-toggle"><button type="button" className={viewMode === 'list' ? 'active' : ''} aria-label="List view" aria-pressed={viewMode === 'list'} onClick={() => setViewMode('list')}><List size={15} /></button><button type="button" className={viewMode === 'grid' ? 'active' : ''} aria-label="Grid view" aria-pressed={viewMode === 'grid'} onClick={() => setViewMode('grid')}><Grid2X2 size={14} /></button></span></RegisterFilters>
          <TableScroll label="Projects" className={`projects-table-wrap ${viewMode === 'grid' ? 'is-grid' : ''}`}><table className="projects-table"><thead><tr><th>Project</th><th>Client</th><th>Status</th><th>Progress</th><th>Budget used</th><th>Manager</th><th>Sites / goals</th><th aria-label="Actions" /></tr></thead><tbody>{projectRows.map((project) => { const budget = Number(project.budget_revised || project.budget || 0); const used = Number(project.budget_actual_expenditure || 0) + Number(project.budget_commitments || 0); const condition = projectCondition(project, today); const badgeTone = condition === 'delayed' ? 'danger' : condition === 'at-risk' ? 'warning' : project.status === 'active' ? 'success' : project.status === 'planning' ? 'info' : statusTone(project.status); const statusLabel = condition === 'delayed' ? 'Delayed' : condition === 'at-risk' ? 'At risk' : project.status_display; return <tr key={project.id}><td><span className="projects-project-cell"><span className="projects-row-icon"><Building2 size={15} /></span><span><Link className="projects-name" to={`/projects/${project.id}/progress`}>{project.name}</Link><small>{project.description || project.code}</small></span></span></td><td>{project.client || '-'}</td><td><Badge tone={badgeTone}>{statusLabel}</Badge></td><td><Link className={`projects-progress ${condition}`} to={`/projects/${project.id}/progress`}><b>{project.progress_percent}%</b><span><i style={{ width: `${project.progress_percent}%` }} /></span></Link></td><td><span className="projects-budget-text">{formatCompactUGX(used)} / {formatCompactUGX(budget)}</span></td><td>{project.manager_name || '-'}</td><td><Link to={`/projects/${project.id}`}>{project.site_total || 0} / {project.goal_total || 0}</Link></td><td><Link className="projects-action" aria-label={`Open actions for ${project.name}`} to={`/projects/${project.id}`}><MoreVertical size={16} /></Link></td></tr>; })}</tbody></table>{!projectRows.length && <p className="projects-empty">{projects.isLoading ? 'Loading projects…' : 'No projects found.'}</p>}</TableScroll>
          <Pagination page={list.page} setPage={list.setPage} data={projects.data} pageSize={4} loading={projects.isFetching} itemLabel="projects" />
        </div>
