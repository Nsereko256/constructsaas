...
export function PurchaseOrderDetailPage() {
  ...
  return <main className="po-detail-page">
    ...
    <div className="po-detail-layout"><div className="po-detail-main">
      <Card title="Order summary"><div className="po-detail-summary"><div><span>Order value</span><strong>{formatUGX(record.total_cost)}</strong></div><div><span>Delivery route</span><strong>{record.delivery_destination_display}</strong></div><div><span>Receipt progress</span><strong>{progress}%</strong></div></div><div className="po-detail-primary"><div><strong>Next action</strong><p>{nextAction}</p></div><Button size="sm" asChild><Link to={continueHref}>{continueLabel}</Link></Button></div></Card>
      <Card title="Ordered materials"><div className="po-detail-table-wrap"><table><thead><tr><th>Material</th><th>Specification</th><th>Ordered</th><th>Unit price</th><th>Line total</th><th>Received</th><th>Outstanding</th><th>Status</th></tr></thead><tbody>{record.items.map((item) => { const accepted = quantities.received.get(item.id) || 0; const rejected = quantities.rejected.get(item.id) || 0; const outstanding = Math.max(0, Number(item.quantity) - accepted); return <tr key={item.id}><td><strong>{item.material_name}</strong><small>{item.material_code || 'Material'}</small></td><td>{item.notes || 'No specification'}</td><td>{formatNumber(item.quantity)} {item.unit}</td><td>{formatUGX(item.unit_price)}</td><td>{formatUGX(item.line_total)}</td><td>{formatNumber(accepted)}<small>{rejected ? `${formatNumber(rejected)} rejected/damaged` : 'Accepted'}</small></td><td>{formatNumber(outstanding)} {item.unit}</td><td><Badge tone={outstanding === 0 ? 'success' : accepted > 0 ? 'warning' : 'info'}>{outstanding === 0 ? 'Received' : accepted > 0 ? 'Part received' : 'Awaiting receipt'}</Badge></td></tr>; })}</tbody><tfoot><tr><td colSpan={4}>Final total</td><td>{formatUGX(record.total_cost)}</td><td colSpan={3} /></tr></tfoot></table></div></Card>
      ...
      <Card title="Source and purpose"><p className="po-detail-source">{record.purchase_request ? <Link to={`/procurement/requests/${record.purchase_request}/`}>{record.purchase_request_number}</Link> : 'No linked purchase request'} · source purchase request</p><p className="po-detail-text">{sourceRequest.data?.justification || record.notes || 'No purpose or procurement notes recorded.'}</p></Card>
    </div><aside className="po-detail-side">
      <Card title="Approval and finance"><div className="po-detail-side-list"><Meta label="Approval status" value={record.status_display} /><Meta label="Finance review" value={record.finance_status_display || (user?.soft_finance_enabled ? 'Ready for handoff' : 'Not enabled')} /><Meta label="Order value" value={formatUGX(record.total_cost)} /></div>{record.status === 'PENDING' && record.purchase_request ? <Button variant="secondary" asChild><Link to={`/procurement/requests?search=${encodeURIComponent(record.purchase_request_number || '')}`}>View finance decision</Link></Button> : null}{user?.soft_finance_enabled ? <p className="po-detail-info">Invoices reduce the commitment; posted payments settle the liability without duplicating expenditure.</p> : null}</Card>
      <Card title="Related records"><div className="po-detail-links"><Link to={`/procurement/requests/${record.purchase_request}/`}><FileText size={16} />Source purchase request</Link><Link to="/procurement/deliveries"><Truck size={16} />Deliveries</Link><Link to="/procurement/grns"><PackageCheck size={16} />Goods received notes <b>{receiptRows.length}</b></Link><Link to="/inventory/movements"><Box size={16} />Stock movements</Link></div></Card>
