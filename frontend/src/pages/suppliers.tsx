...
export function SuppliersPage() {
  ...
  return (
    <div className="operations-reference suppliers-reference grid gap-3">
      ...
      <div className="ops-workspace"><section data-pagination-region className="ops-register"><div className="ops-register-head"><h2>Supplier directory</h2></div><div className="ops-register-tabs"><button className={view === 'all' ? 'active' : ''} onClick={() => { setView('all'); list.setFilter('is_active', ''); list.setFilter('is_preferred', ''); }}>All <small>{directory.length}</small></button><button className={view === 'active' ? 'active' : ''} onClick={() => { setView('active'); list.setFilter('is_active', 'true'); list.setFilter('is_preferred', ''); }}>Active <small>{active.length}</small></button><button className={view === 'preferred' ? 'active' : ''} onClick={() => { setView('preferred'); list.setFilter('is_active', 'true'); list.setFilter('is_preferred', 'true'); }}>Preferred <small>{preferred.length}</small></button><button className={view === 'inactive' ? 'active' : ''} onClick={() => { setView('inactive'); list.setFilter('is_active', 'false'); list.setFilter('is_preferred', ''); }}>Inactive <small>{directory.length - active.length}</small></button></div><RegisterFilters className="ops-filters" activeCount={[list.search, list.filters.rating, list.filters.is_active, list.filters.is_preferred].filter(Boolean).length} onClear={() => { setView('all'); list.setSearch(''); ['rating', 'is_active', 'is_preferred'].forEach((key) => list.setFilter(key, '')); }}><input aria-label="Search suppliers" className={inputClass} value={list.search} onChange={(event) => list.setSearch(event.target.value)} placeholder="Search supplier, contact or address" /><select aria-label="Filter suppliers by rating" className={inputClass} value={list.filters.rating} onChange={(event) => list.setFilter('rating', event.target.value)}><option value="">All ratings</option>{[5,4,3,2,1].map((rating) => <option key={rating} value={rating}>{rating} stars</option>)}</select><select aria-label="Filter suppliers by activity" className={inputClass} value={list.filters.is_active} onChange={(event) => { const value = event.target.value; setView(value === 'true' ? 'active' : value === 'false' ? 'inactive' : 'all'); list.setFilter('is_preferred', ''); list.setFilter('is_active', value); }}><option value="">All statuses</option><option value="true">Active</option><option value="false">Inactive</option></select></RegisterFilters><DataTable columns={columns} data={visibleSuppliers} mobileSummaryCells={2} emptyTitle={suppliers.isLoading ? 'Loading suppliers...' : 'No suppliers found'} /><Pagination itemLabel="suppliers" loading={suppliers.isFetching} page={list.page} setPage={list.setPage} data={suppliers.data} /></section>
        <Dialog open={!!selected} onOpenChange={(value) => !value && setSelected(null)}><DialogContent title={selected?.name || 'Supplier details'} description="Contact information and procurement performance." className="supplier-inspector"><div className="grid gap-4">{focused ? <section className="grid gap-2 border-b border-border pb-4 text-sm"><p>{focused.contact_person || 'No contact person recorded'}</p><p>{focused.phone || 'No phone recorded'}</p><p className="break-all">{focused.email || 'No email recorded'}</p><p>{focused.address || 'No address recorded'}</p></section> : null}{focused ? <><section className="ops-panel"><div className="ops-panel-head"><h2>Supplier performance</h2></div><div className="ops-panel-body"><p className="mb-1 text-xs font-bold">{focused.name}</p><OpsRow icon={PackageCheck} label="Deliveries" value={supplierMetrics.deliveries} /><OpsRow icon={Clock3} label="On-time delivery" value={supplierMetrics.onTime === null ? 'Not measured' : `${supplierMetrics.onTime}%`} /><OpsRow icon={CheckCircle2} label="Receipt acceptance" value={supplierMetrics.quality === null ? 'Not measured' : `${supplierMetrics.quality}%`} /><OpsRow icon={Truck} label="Configured lead time" value={focused.lead_time_days ? `${focused.lead_time_days} days` : 'Not set'} /></div></section><section className="ops-panel"><div className="ops-panel-head"><h2>Procurement activity</h2></div><div className="ops-panel-body"><OpsRow icon={PackageCheck} label="Purchase orders" value={supplierMetrics.orders} /><OpsRow icon={Clock3} label="Open orders" value={supplierMetrics.openOrders} /><OpsRow icon={Coins} label="Total order value" value={formatUGX(supplierMetrics.orderValue)} /></div></section><section className="ops-panel"><div className="ops-panel-head"><h2>Profile completeness</h2></div><div className="ops-panel-body"><OpsRow icon={Phone} label="Phone number" value={focused.phone ? 'Provided' : 'Missing'} /><OpsRow icon={Mail} label="Email address" value={focused.email ? 'Provided' : 'Missing'} /><OpsRow icon={MapPin} label="Address" value={focused.address ? 'Provided' : 'Missing'} /><OpsRow icon={CheckCircle2} label="Compliance record" value={focused.compliance_reference ? 'Provided' : 'Missing'} /><div className="mt-3 flex items-center justify-between text-xs"><span>Completion</span><strong>{profileCompleteness(focused)}%</strong></div><div className="ops-progress mt-1"><span style={{ width: `${profileCompleteness(focused)}%` }} /></div>{allowed ? <Button className="mt-3 w-full" variant="secondary" size="sm" onClick={() => { setSelected(null); setOpen(focused); }}>Edit supplier</Button> : null}</div></section></> : <section className="ops-panel"><div className="ops-panel-body text-xs text-muted">Select a supplier to review performance.</div></section>}</div></DialogContent></Dialog>
      </div>
