...
export function SuppliersPage() {
  const { role } = useAuth();
  const list = useListState({ rating: '', is_active: '' });
  ...
  const visibleSuppliers = view === 'preferred' ? (suppliers.data?.results || []).filter((supplier) => supplier.is_preferred) : suppliers.data?.results || [];
  ...
  const columns: ColumnDef<Supplier>[] = [
    ...
    { header: 'Status', cell: ({ row }) => <Badge tone={row.original.is_active ? 'success' : 'neutral'}>{row.original.is_active ? row.original.is_preferred ? 'Preferred' : 'Active' : 'Inactive'}</Badge> },
    {
      ...
      header: '',
      cell: ({ row }) =>
        <div className="flex flex-wrap justify-end gap-1"><Button variant="ghost" size="sm" onClick={() => setSelected(row.original)}><Eye className="h-4 w-4" />View</Button>{allowed ? <><Button variant="ghost" size="sm" onClick={() => setOpen(row.original)}><Pencil className="h-4 w-4" />Edit</Button>{row.original.is_active ? <Button variant="ghost" size="sm" onClick={() => { if (window.confirm(`Deactivate ${row.original.name}? Existing procurement history will remain available.`)) deactivate.mutate(row.original.id); }}><Trash2 className="h-4 w-4" />Deactivate</Button> : null}</> : null}</div>,
    },
  ];
  ...
  return (
    <div className="operations-reference grid gap-3">
      ...
      <div className="ops-workspace"><section className="ops-register"><div className="ops-register-head"><h2>Supplier directory</h2></div><div className="ops-register-tabs"><button className={view === 'all' ? 'active' : ''} onClick={() => { setView('all'); list.setFilter('is_active', ''); }}>All <small>{directory.length}</small></button><button className={view === 'active' ? 'active' : ''} onClick={() => { setView('active'); list.setFilter('is_active', 'true'); }}>Active <small>{active.length}</small></button><button className={view === 'preferred' ? 'active' : ''} onClick={() => { setView('preferred'); list.setFilter('is_active', 'true'); }}>Preferred <small>{preferred.length}</small></button><button className={view === 'inactive' ? 'active' : ''} onClick={() => { setView('inactive'); list.setFilter('is_active', 'false'); }}>Inactive <small>{directory.length - active.length}</small></button></div><div className="ops-filters"><input className={inputClass} value={list.search} onChange={(event) => list.setSearch(event.target.value)} placeholder="Search supplier, contact or address" /><select className={inputClass} value={list.filters.rating} onChange={(event) => list.setFilter('rating', event.target.value)}><option value="">All ratings</option>{[5,4,3,2,1].map((rating) => <option key={rating} value={rating}>{rating} stars</option>)}</select><select className={inputClass} value={list.filters.is_active} onChange={(event) => list.setFilter('is_active', event.target.value)}><option value="">All statuses</option><option value="true">Active</option><option value="false">Inactive</option></select></div><DataTable columns={columns} data={visibleSuppliers} mobileSummaryCells={2} emptyTitle={suppliers.isLoading ? 'Loading suppliers...' : 'No suppliers found'} /><Pagination page={list.page} setPage={list.setPage} data={suppliers.data} /></section>
