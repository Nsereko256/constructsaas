...
export function PurchaseRequestDetailPage() {
  ...
  return <main className="pr-detail-page">
    ...
    <div className="pr-detail-layout"><div className="pr-detail-main">
      ...
      <PrCard title="Related records"><div className="pr-detail-related"><Link to="/inventory/movements"><Box size={17} /><span>Stock issue<small>Not created</small></span><ChevronRight size={16} /></Link><Link to="/procurement/rfqs"><FileText size={17} /><span>RFQ<small>Not required</small></span><ChevronRight size={16} /></Link><Link to={`/procurement/purchase-orders?purchase_request=${record.id}`}><ClipboardList size={17} /><span>Purchase order<small>{orders.data?.count ? `${orders.data.count} linked` : 'Not required'}</small></span><ChevronRight size={16} /></Link><button type="button" disabled><Paperclip size={17} /><span>Attachments<small>0 files</small></span><ChevronRight size={16} /></button></div></PrCard>
    </div><aside className="pr-detail-side">
      <PrCard title="Approval workflow"><ApprovalWorkflow request={record} /></PrCard>
      {softFinance ? <PrCard title="Soft Finance"><div className="pr-detail-finance"><PrMeta label="Budget line" value={record.finance_budget_line ? `Budget line #${record.finance_budget_line}` : 'Site preparation'} /><PrMeta label="Available project budget" value="UGX 12.0M" /><PrMeta label="Request impact" value={formatUGX(record.total_estimated_cost)} /><PrMeta label="After approval" value="UGX 11.8M" /><PrMeta label="Status" value={<span className="pr-detail-green">● Within budget</span>} /></div><p className="pr-detail-info"><CircleDollarSign size={15} />A PR estimate is not a commitment.</p></PrCard> : null}
