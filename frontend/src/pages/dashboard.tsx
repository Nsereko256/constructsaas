      ...
      <section className="grid gap-3 sm:gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        <Card>
          ...
          <CardContent className="p-3 sm:p-4">
            ...
            {budgetRows.map((project) => <Link key={project.id} to={`/projects/${project.id}/progress`} className="grid grid-cols-[minmax(92px,0.32fr)_minmax(0,1fr)_38px] items-center gap-2 rounded-lg px-1 py-1 transition hover:bg-primary/[0.035] sm:grid-cols-[minmax(150px,0.34fr)_minmax(0,1fr)_42px] sm:gap-3"><span className="min-w-0"><strong className="block truncate text-xs font-semibold text-foreground sm:text-sm">{project.name}</strong><span className="block truncate text-[10px] text-muted">{project.code}</span></span><span className="grid gap-1.5"><span className="relative block h-2.5 overflow-hidden rounded-full bg-[#F0F3F3]"><span className="absolute inset-y-0 left-0 rounded-full" style={{ width: `${project.plannedProgress}%`, minWidth: project.plannedProgress > 0 ? 4 : 0, backgroundColor: '#C9D0D1' }} /></span><span className="relative block h-2.5 overflow-hidden rounded-full bg-[#F0F3F3]"><span className={`absolute inset-y-0 left-0 rounded-full ${project.atRisk ? 'bg-critical' : ''}`} style={{ width: `${project.actualProgress}%`, minWidth: project.actualProgress > 0 ? 4 : 0, backgroundColor: project.atRisk ? undefined : '#0F7075' }} /></span></span><span className={`text-right text-xs font-bold ${project.actualProgress < project.plannedProgress ? 'text-warning' : 'text-[#0F7075]'}`}>{project.actualProgress}%</span></Link>)}
