      ...
      <section className="grid gap-3 sm:gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        <Card>
          ...
          <CardContent className="p-3 sm:p-4">
            ...
            {budgetRows.map((project, index) => { const tones = [{ actual: '#0F7075', planned: '#BFE3E2' }, { actual: '#2878D0', planned: '#C9DDF4' }, { actual: '#7C4DAD', planned: '#E0D2F0' }, { actual: '#D98B16', planned: '#F4DDAF' }, { actual: '#D24C5A', planned: '#F2C7CD' }, { actual: '#2C8A57', planned: '#C9E6D5' }]; const tone = tones[index % tones.length]; return <Link key={project.id} to={`/projects/${project.id}/progress`} className="grid grid-cols-[minmax(92px,0.32fr)_minmax(0,1fr)_38px] items-center gap-2 rounded-xl border border-transparent px-2 py-1.5 transition hover:border-border hover:bg-background sm:grid-cols-[minmax(150px,0.34fr)_minmax(0,1fr)_42px] sm:gap-3"><span className="flex min-w-0 items-center gap-2"><i className="h-2.5 w-2.5 shrink-0 rounded-full shadow-sm" style={{ backgroundColor: project.atRisk ? '#D24C5A' : tone.actual }} /><span className="min-w-0"><strong className="block truncate text-xs font-semibold text-foreground sm:text-sm">{project.name}</strong><span className="block truncate text-[10px] text-muted">{project.code}</span></span></span><span className="grid gap-1.5"><span className="relative block h-2.5 overflow-hidden rounded-full bg-[#F0F3F3]"><span className="absolute inset-y-0 left-0 rounded-full" style={{ width: `${project.plannedProgress}%`, minWidth: project.plannedProgress > 0 ? 4 : 0, backgroundColor: tone.planned }} /></span><span className="relative block h-2.5 overflow-hidden rounded-full bg-[#F0F3F3]"><span className="absolute inset-y-0 left-0 rounded-full" style={{ width: `${project.actualProgress}%`, minWidth: project.actualProgress > 0 ? 4 : 0, backgroundColor: project.atRisk ? '#D24C5A' : tone.actual }} /></span></span><span className={`text-right text-xs font-bold ${project.actualProgress < project.plannedProgress ? 'text-warning' : ''}`} style={{ color: project.atRisk ? '#D24C5A' : tone.actual }}>{project.actualProgress}%</span></Link>; })}
