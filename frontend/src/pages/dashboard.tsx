  ...
  return (
    <div className="grid gap-2.5 sm:gap-5">
      ...
      <section aria-label="Operations pipeline" className="rounded-2xl border border-border bg-white p-3 shadow-panel sm:p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-3 lg:grid-cols-6">{pipeline.map(({ label, count, href, icon: Icon }, index) => <Link key={label} to={href} className={`group relative overflow-hidden rounded-2xl border bg-background p-3.5 transition-all hover:-translate-y-0.5 hover:shadow-lift ${count ? 'border-warning/35 hover:border-warning' : 'border-border hover:border-primary/40'}`}><span className={`absolute inset-x-0 top-0 h-1 ${count ? 'bg-warning' : 'bg-primary/35 group-hover:bg-primary'}`} /><div className="flex items-start justify-between gap-2"><span className={`grid h-8 w-8 place-items-center rounded-xl ${count ? 'bg-warning/15 text-warning' : 'bg-primary/10 text-primary'}`}><Icon className="h-4 w-4" /></span><span className="text-[10px] font-black text-muted">0{index + 1}</span></div><p className="mt-3 text-[10px] font-black uppercase tracking-[0.1em] text-muted">{label}</p><strong className="mt-0.5 block text-2xl font-black tracking-tight">{count}</strong><span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[10px] font-bold ${count ? 'bg-warning/15 text-warning' : 'bg-success/10 text-success'}`}>{count ? 'Needs attention' : 'All clear'}</span>{index < 5 ? <span className="absolute -right-1.5 top-1/2 z-10 hidden h-3 w-3 -translate-y-1/2 rotate-45 border-r border-t border-border bg-background lg:block" /> : null}</Link>)}
        </div>
      </section>
