...
import { AlertTriangle, Bell, BellRing, Check, CheckCheck, CircleAlert, FileText, Info, Mail, PackageCheck, Search, Settings, WalletCards, X } from 'lucide-react';
...
export function NotificationsPage() {
  ...
  const emailPreferences = useQuery({ queryKey: ['notifications', 'email-preferences'], queryFn: api.emailNotificationPreferences });
  ...
  return (
    <div className="notifications-reference">
      ...
      <section className="notifications-kpis" aria-label="Notification summary">
        <NotificationKpi icon={Bell} tone="teal" label="Unread" value={summary.isError ? '—' : stats?.unread ?? 0} detail={summary.isError ? 'Unavailable' : `${stats?.today ?? 0} new today`} />
        <NotificationKpi icon={AlertTriangle} tone="amber" label="Action required" value={summary.isError ? '—' : stats?.action_required ?? 0} detail={summary.isError ? 'Unavailable' : 'Needs your attention'} />
        <NotificationKpi icon={Check} tone="green" label="Approvals" value={summary.isError ? '—' : stats?.approvals ?? 0} detail={summary.isError ? 'Unavailable' : 'Awaiting approval'} />
      ...
      <div className="notifications-workspace-grid">
        <section data-pagination-region className="notifications-register">
          ...
          <RegisterFilters className="notifications-filters" activeCount={[list.search, list.filters.is_read].filter(Boolean).length} onClear={() => { list.setSearch(''); list.setFilter('is_read', ''); }}><label><Search className="h-4 w-4" /><input value={list.search} onChange={(event) => list.setSearch(event.target.value)} placeholder="Search notifications" aria-label="Search notifications" />{list.search ? <button type="button" aria-label="Clear notification search" onClick={() => list.setSearch('')}><X className="h-3.5 w-3.5" /></button> : null}</label><select value={list.filters.is_read} onChange={(event) => list.setFilter('is_read', event.target.value)} aria-label="Notification status"><option value="">All notifications</option><option value="false">Unread only</option><option value="true">Read only</option></select></RegisterFilters>
          <div className="notifications-list">
            {notifications.isError ? <p className="notifications-empty">The notification list is temporarily unavailable.</p> : null}
        ...
        <aside className="notifications-side-column">
          ...
          <section className="notifications-side-panel notifications-delivery-panel"><div className="notifications-panel-heading"><h2>Delivery preferences</h2><Link to="/settings">Manage</Link></div><div className="notifications-preference-row"><span className="notifications-preference-icon"><BellRing className="h-4 w-4" /></span><span><strong>Push notifications</strong><small>Receive browser alerts</small></span><Badge tone={pushEnabled ? 'success' : 'neutral'}>{pushEnabled ? 'On' : 'Off'}</Badge></div><div className="notifications-preference-actions"><Button size="sm" variant={pushEnabled ? 'secondary' : 'default'} onClick={() => void togglePush()} disabled={pushBusy || !pushAvailable}>{pushEnabled ? 'Turn off' : 'Turn on'}</Button>{pushEnabled ? <Button size="sm" variant="ghost" onClick={() => void testPush()} disabled={pushBusy}>Test</Button> : null}{!pushAvailable ? <small>Browser push is unavailable.</small> : null}</div><div className="notifications-preference-row"><span className="notifications-preference-icon"><Mail className="h-4 w-4" /></span><span><strong>Email alerts</strong><small>{emailPreferences.data?.required_only ? 'Required actions only' : 'Selected activity'}</small></span><Badge tone={emailPreferences.data?.enabled && emailPreferences.data?.has_email && emailPreferences.data?.provider_configured ? 'success' : 'neutral'}>{emailPreferences.data?.enabled && emailPreferences.data?.has_email && emailPreferences.data?.provider_configured ? 'On' : 'Off'}</Badge></div></section>
        </aside>
      </div>
...
function NotificationKpi({ icon: Icon, tone, label, value, detail }: { icon: typeof Bell; tone: string; label: string; value: number | string; detail: string }) { return <div className={`notifications-kpi ${tone}`}><span className="notifications-kpi-icon"><Icon className="h-6 w-6" /></span><span><p>{label}</p><strong>{value}</strong><small>{detail}</small></span></div>; }
...
function notificationActionLabel(type: string) { if (type === 'po_received') return 'Open receipt'; if (type === 'low_stock') return 'Open inventory'; if (type.startsWith('invoice_')) return 'Open invoice actions'; if (type.startsWith('payment_')) return 'Open payment actions'; if (type.startsWith('pr_')) return 'Open material request'; if (type.startsWith('po_')) return 'Open purchase order'; if (type.includes('budget')) return 'Open budget review'; if (type.includes('valuation')) return 'Open inventory movements'; return 'Open related work'; }
