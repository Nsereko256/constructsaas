...
export function PurchaseOrderDetailPage() {
  ...
  return <RecordShell backTo="/procurement/purchase-orders" label="purchase orders">
    ...
    <section className="record-detail-primary"><div><strong>Primary next action</strong><p>{record.status === 'PENDING' ? 'Send the quoted order to Finance for review.' : record.status === 'ORDERED' ? 'Confirm supplier dispatch or follow up delivery.' : record.status === 'PARTIAL' ? 'Record the remaining receipt.' : 'Monitor the order progress and evidence.'}</p></div>{record.purchase_request ? <Button asChild><Link to={`/procurement/requests/${record.purchase_request}/`}>View source PR</Link></Button> : null}</section>
