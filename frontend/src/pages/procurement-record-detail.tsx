...
export function PurchaseRequestDetailPage() {
  ...
  if (request.isLoading) return <LoadingRecord label="material requests" backTo="/procurement/requests" />;
  if (request.isError || !request.data) return <RecordNotFound label="Material request" backTo="/procurement/requests" />;
  ...
  return <RecordShell backTo="/procurement/requests" label="material requests">
    <section className="record-detail-hero"><div><p>Procurement / Material request</p><h1>{record.number}</h1><span>{record.title}</span></div><div className="record-detail-badges"><Badge tone={statusTone(record.priority)}>{record.priority_display}</Badge><Badge tone={statusTone(record.status)}>{record.status_display}</Badge></div></section>
