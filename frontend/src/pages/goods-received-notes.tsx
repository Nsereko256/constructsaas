...
function ReceiptRow({ row, onView, onDownload }: { row: ReceiptSummary; onView: () => void; onDownload: () => void }) {
  ...
  return <tr><td data-label="GRN"><button type="button" className="grn-document-link" onClick={onView}>{row.note.number}</button><small>{row.note.items.length} material{row.note.items.length === 1 ? '' : 's'}</small></td><td data-label="Next action"><Button size="sm" variant="ghost" className="grn-view-action" onClick={onView}><Eye size={13} />View</Button></td><td data-label="Purchase order"><Link to={`/procurement/purchase-orders?search=${encodeURIComponent(row.note.purchase_order_number)}`}>{row.note.purchase_order_number}</Link></td><td data-label="Supplier">{row.order?.supplier_name || 'Supplier not recorded'}</td><td data-label="Destination"><Badge tone={row.order?.delivery_destination === 'SITE' ? 'info' : 'success'}>{row.order?.delivery_destination_display || 'Destination unavailable'}</Badge></td><td data-label="Received">{formatDate(row.note.receipt_date)}<small>{formatNumber(row.accepted)} accepted</small></td><td data-label="Receiver">{row.note.received_by_name || row.note.received_by_username || 'Recorded receiver'}</td><td data-label="Materials">{row.note.items.length} material{row.note.items.length === 1 ? '' : 's'}<small><button type="button" onClick={onView}>View lines</button></small></td><td data-label="Condition"><Badge tone={row.hasException ? 'warning' : 'success'}>{condition}</Badge></td><td data-label="Stock posting"><Badge tone={row.note.status === 'ACCEPTED' && row.accepted > 0 ? 'success' : 'neutral'}>{row.note.status === 'ACCEPTED' && row.accepted > 0 ? 'Posted' : 'Not posted'}</Badge></td><td data-label="Status"><Badge tone={statusTone(row.note.status)}>{row.note.status === 'ACCEPTED' ? 'Accepted' : 'Reversed'}</Badge></td><td data-label=""><details className="grn-row-menu"><summary aria-label={`More actions for ${row.note.number}`}><EllipsisVertical size={16} /></summary><div><button type="button" onClick={onView}><Eye size={13} />View receipt</button><button type="button" onClick={onDownload}><Download size={13} />Download PDF</button></div></details></td></tr>;
}
