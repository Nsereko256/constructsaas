      ...
      <section className="grid place-items-center p-5">
        <Card className="reference-auth-card w-full max-w-md p-6">
          ...
          <form className="reference-auth-form mt-6 grid gap-4" onSubmit={form.handleSubmit(onSubmit)}>
            ...
            {sessionConflict ? <div className="grid gap-3 rounded-xl border border-warning/30 bg-warning/10 p-3 text-sm"><div><strong>Account active elsewhere</strong><p className="mt-1 text-muted">This account is currently active in another browser or device. Continue only if you want to end that session.</p></div><div className="flex flex-col gap-2 sm:flex-row"><Button type="button" onClick={() => void endOtherSession()}>End other session and continue</Button><Button type="button" variant="ghost" onClick={() => setSessionConflict(null)}>Cancel</Button></div></div> : null}
