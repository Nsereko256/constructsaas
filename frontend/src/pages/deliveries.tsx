...
export function DeliveriesPage() {
  ...
  const refresh = async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: ['goods-received-notes'], refetchType: 'active' }),
      queryClient.invalidateQueries({ queryKey: qk.dashboard, refetchType: 'active' }),
      queryClient.invalidateQueries({ queryKey: qk.workflowBadges, refetchType: 'active' }),
      queryClient.invalidateQueries({ queryKey: ['finance'], refetchType: 'active' }),
    ]);
  };
  const dispatch = useMutation({
    mutationFn: api.confirmDispatch,
    onSuccess: async () => { await refresh(); toast.push({ title: 'Dispatch confirmed', tone: 'success' }); },
  ...
  const receive = useMutation({
    ...
    onSuccess: async (order) => {
      if ('queued' in order) {
        ...
        await refresh();
        toast.push({ title: replacementClaimId ? 'Supplier replacement received and claim resolved' : order.status === 'PARTIAL' ? 'Partial receipt recorded' : 'Receipt confirmed', message: order.status === 'RECEIVED' ? 'The delivery is complete and its GRN is now available in Receipts.' : 'The remaining quantity stays in this queue.', tone: 'success' });
      }
      setReceiving(null);
      ...
      <div className="grid gap-3">
        {lines.map((line) => {
          ...
          return <div key={item.id} className="grid gap-3 border border-border p-3 md:grid-cols-[minmax(180px,1.5fr)_repeat(3,110px)]">
            <div><strong>{item.material_name}</strong><p className="text-xs text-muted">Ordered: {item.quantity} / remaining to receive: {remaining}</p><p className="mt-1 text-xs text-muted">Finance-approved unit price: <b className="text-foreground">{formatUGX(item.unit_price)}</b>{Number(line.accepted_quantity) > 0 ? ` · accepted value ${formatUGX(Number(line.accepted_quantity) * Number(item.unit_price))}` : ''}</p><input className={`${inputClass} mt-2`} value={line.notes} onChange={(event) => update('notes', event.target.value)} placeholder={needsReason ? 'Reason for rejection/damage (required)' : 'Line note'} /><p className="mt-1 text-xs text-danger">{needsReason && !line.notes.trim() ? 'Reason required for rejected or damaged goods.' : ''}</p></div>
