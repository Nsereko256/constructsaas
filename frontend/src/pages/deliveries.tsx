...
export function DeliveriesPage() {
  ...
  return <div className="deliveries-reference"><section className="del-top"><div className="del-titlebar"><div><h1>Deliveries</h1><p>Track supplier dispatches, expected arrivals and receipt confirmation.</p></div><div className="del-title-actions"><details className="del-export-menu"><summary><Download size={15} />Export <ChevronDown size={13} /></summary><div><button type="button" onClick={() => void exportRegister('pdf')}>PDF register</button><button type="button" onClick={() => void exportRegister('xlsx')}>Excel register</button></div></details><Button asChild><Link to={can.createPo(role) ? '/procurement/deliveries?status=ORDERED&delivery_destination=SITE' : '/procurement/deliveries'}>{can.createPo(role) ? <><Truck className="h-4 w-4" />Record dispatch</> : <><PackageCheck className="h-4 w-4" />Delivery queue</>}</Link></Button></div></div><nav className="del-tabs" aria-label="Procurement sections"><Link to="/procurement">Overview</Link><Link to="/procurement/requests">Material requests</Link><Link to="/procurement/purchase-orders">Purchase orders</Link><Link to="/procurement/grns">Receipts</Link><Link className="active" to="/procurement/deliveries">Deliveries</Link></nav></section>
