    ...
    <FormModal open={open} title={`Approve ${recordNumber || 'purchase request'}`} onClose={onClose}>
      <form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); if (valid) onApprove(reason.trim()); }}>
        <div className="flex gap-3 rounded-lg border border-warning/35 bg-warning/5 p-4 text-sm">
          ...
          <div><strong>Controlled administrator override</strong><p className="mt-1 text-muted">You prepared this request and are now approving it. Record why the normal maker-checker separation cannot be used. This reason is retained in the audit trail.</p></div>
        </div>
        ...
        <div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button loading={pending} loadingLabel="Approving request" disabled={!valid}>Approve with recorded override</Button></div>
      </form>
    </FormModal>
  );
}
