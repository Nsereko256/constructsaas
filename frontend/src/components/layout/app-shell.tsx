...
export function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 flex w-[min(82vw,296px)] flex-col border-r border-sidebar-border bg-sidebar py-4 text-white transition-all md:w-auto md:translate-x-0',
          collapsed ? 'w-[76px]' : 'w-[248px]',
          mobileOpen ? 'translate-x-0' : '-translate-x-full',
        )}
      >
        <div className="flex items-center gap-2.5 px-3">
          <div className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-primary text-white"><HardHat className="h-4 w-4" /></div>
          {!collapsed ? (
            <div className="min-w-0">
              <strong className="block truncate font-display text-sm">ConstructSaaS</strong>
              <span className="block truncate text-[11px] text-white/60">{user?.company_name || 'Operations control'}</span>
            </div>
          ) : null}
        </div>
        <nav className="mt-6 grid gap-1 overflow-auto px-2 pb-4 scrollbar-thin">
          {nav.map((item, index) => {
            ...
            <div key={item.href}>
              {!collapsed && item.section && item.section !== nav[index - 1]?.section ? (
                <p className="mb-1 mt-4 px-3 text-[11px] font-semibold uppercase tracking-widest text-white/50 first:mt-0">{item.section}</p>
              ) : null}
            <NavLink
              to={item.href}
              ...
              className={({ isActive }) =>
                cn(
                'relative flex min-h-11 items-center gap-2.5 rounded-md px-3 py-2 text-sm text-white/85 transition-colors hover:bg-white/10 hover:text-white',
      ...
      <div className={cn('transition-all', collapsed ? 'md:pl-[76px]' : 'md:pl-[248px]')}>
        <header className="sticky top-0 z-30 flex min-h-[60px] items-center gap-2 border-b border-border bg-white/95 px-3 backdrop-blur sm:gap-3 sm:px-4">
          ...
          <div className="ml-auto min-w-0 truncate text-xs text-muted sm:text-sm">
            Signed in as <span className="font-semibold text-foreground">{user?.role_display || 'User'}</span>
          </div>
          ...
          <div className="hidden text-right text-sm lg:block"><strong className="block">{user?.username}</strong><span className="inline-flex items-center gap-1 text-muted"><ShieldCheck className="h-3.5 w-3.5" />{user?.role_display}</span></div>
