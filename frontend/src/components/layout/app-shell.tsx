    ...
    <div className="professional-workspace min-h-screen bg-background text-foreground">
      ...
      <div className={cn('transition-all', collapsed ? 'md:pl-[62px]' : 'md:pl-[170px]')}>
        <header className="sticky top-0 z-30 flex min-h-[48px] items-center gap-2 border-b border-border bg-white/95 px-3 backdrop-blur sm:gap-3 sm:px-5">
          ...
          {!online ? <div className="hidden h-9 items-center gap-1.5 border border-[#E5CC8E] bg-[#FFF8E7] px-2.5 text-xs font-medium text-[#7A5D1B] sm:inline-flex"><WifiOff className="h-3.5 w-3.5" />Offline</div> : null}
          ...
          <div className="hidden items-center gap-2 lg:flex"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#5C322E] text-xs font-bold text-white">{(user?.first_name || user?.username || 'U').slice(0, 1).toUpperCase()}</span><strong className="text-xs">{user?.username}</strong></div>
